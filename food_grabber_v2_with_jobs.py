...
def valid_neighbors(row: int, col: int, walls: npt.NDArray[np.int_]) -> list[Point]:
    """
...
def move_towards_dest(a: Point, b: Point, walls: npt.NDArray[np.int_]) -> Point:
...
class FoodBot2:
    ...
    def __init__(
        ...
        self.previous_vision = set()

        self.run_count = 1
        self.friendly_ant_goal = defaultdict(
            lambda: random.choices(
                [Job.Food, Job.Defend, Job.Attack], weights=[0.3, 0.3, 0.4]
            )
        )
    ...
    def process_vision(
        ...
        for item in vision:
            ...
            if entity == Entity.FRIENDLY_ANT:
                self.friendly_ant_goal[coord]
        
        for item in (vision - self.previous_vision):
            if item[1] == Entity.FRIENDLY_ANT:
                del self.friendly_ant_goal[item[0]]
    ...
    def move_ants(
        ...
        for food in foods:
            ...
            for ant in my_ants:
                heapq.heappush(food_and_ant[food], (dist(ant, food, self.walls), ant))
            dis, ant = heapq.heappop(food_and_ant[food])
            heapq.heappush(food_ant_q, (dis, food, ant))
        ...
        while food_ant_q:
            ...
            if ant in claimed_ants:
                if food_and_ant[food]:
                    ...
                    heapq.heappush(food_ant_q, (dis, food, ant2))
            else:
